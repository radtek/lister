...
GridCtrl::ItemRect& GridCtrl::ItemRect::Min(int n)
{
	if(hidden)
		return *this;
	min = n;
	ismin = true; // The reason I cloned GridCtrl into lister:  I need to know if this column is fixed (The fix is in!)
	if(size < min)
		size = min;
	return *this;
}
...
GridCtrl::ItemRect& GridCtrl::ItemRect::Max(int n)
{
	if(hidden)
		return *this;
	max = n;
	ismax = true; // The reason I cloned GridCtrl into lister:  I need to know if this column is fixed (The fix is in!)
	if(size > max)
		size = max;
	return *this;
}
