...
// Added by Jeff in order allow non-blocking asynchronous OCI functions to pass control back
// to main window, specifically to ProcessEvents() ever 10 ms.
void SqlConnection::SetWhenStillExecuting(const Callback &cb) {
	WhenStillExecuting = cb;
}
...
int  SqlConnection::GetParseErrorPosition() const {
	NEVER();
	return 0;
}
...
// Added by Jeff to support callbacks to main window that can then PostEvents() on TopWindow
void Sql::SetWhenStillExecuting(const Callback &cb) { // FYI: cb is option param
	// Seems pointless, but stuck with it.  See Oci8.cpp, it can access the cn object.
	// cn becomes...OCI8Connection!
	cn->SetWhenStillExecuting(cb); 
}
