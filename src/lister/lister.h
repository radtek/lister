...
//		break;
	...
	void MyToolBar(Bar& bar) {
		...
		bar.Add(
			(cmdScript.connection), "ListUsers", MyImages::deploylister16(), 
			 THISBACK(DeployLister)).Tip("List users/schemas for current connection");

		bar.Add(userList, 100);

		bar.Add(true, "Deploy", MyImages::user16(), 
			 THISBACK(ListUsers)).Tip("Deploy lister to program files");
