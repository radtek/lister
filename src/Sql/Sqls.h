...
class SqlConnection {
public:
	Callback WhenStillExecuting; // Called from Execute() and Fetch() if set. (See OCI8Connection in Oci8.h)
...
protected:
	...
	// See Sql.cpp
	virtual void		SetWhenStillExecuting(const Callback &cb = CNULL); // Added by Jeff to support callbacks to main window that can then PostEvents() on TopWindow
...
// The public form of SqlConnection
class Sql {
protected:
	SqlConnection  *cn;
...
public:
	...
	void   SetWhenStillExecuting(const Callback &cb = CNULL); // Added by Jeff to support callbacks to main window that can then PostEvents() on TopWindow
	bool   Execute();
	void   ExecuteX();
